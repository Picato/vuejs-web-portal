  ...
  <div v-if="data">
    ...
    <table class="table is-bordered is-striped is-narrow is-fullwidth" v-else>
      <tr>
        ...
        <th @click="sortLast=sortBid=sortVolume=sortAsk; ++sortPercent;" class="is-cursor">Percent rate
          <span class="icon is-pulled-right">
      ...
      <tr v-for="(vl, k) in data" :key="k">
        ...
        <td>
          <span class="fa" :class="{'fa-long-arrow-up has-text-success': vl.percent > 100, 'fa-long-arrow-down has-text-danger': vl.percent < 100 }" v-show="vl.percent !== 100"></span>&nbsp;{{vl.percent}} %
        </td>
      </tr>
    </table>
  </div>
...
export default {
  ...
  computed: {
    data() {
      ...
      else if (this.sortVolume % 3 === 1) data.sort((a, b) => b.baseVolume.usdt - a.baseVolume.usdt)
